...
describe('zoomAt', () => {
  ...
  it('zoom factor 1.0 leaves zoom unchanged', () => {
    ...
    expect(
      approx(vp.zoomAt(1.0, { x: 400, y: 300 }).getState().zoom, 0.5),
    ).toBe(true);
  });
});
...
describe('fitBounds', () => {
  ...
  it('degenerate single-point bounds: zoom = maxZoom, point centred', () => {
    ...
    expectScreen(
      fitted.tileToScreen({ x: 2048, y: 2048 }),
      { x: W / 2, y: H / 2 },
      1e-6,
    );
  });
  ...
  it('throws on inverted bounding box (minX > maxX)', () => {
    expect(() =>
      vp.fitBounds({ minX: 3000, minY: 0, maxX: 1000, maxY: 1000 }),
    ).toThrow('Invalid BoundingBox');
  });

  it('throws on inverted bounding box (minY > maxY)', () => {
    expect(() =>
      vp.fitBounds({ minX: 0, minY: 3000, maxX: 1000, maxY: 1000 }),
    ).toThrow('Invalid BoundingBox');
  });
});
...
describe('resize', () => {
  it('updates width and height', () => {
    const s = createViewport({ width: W, height: H })
      .resize(1024, 768)
      .getState();
  ...
  it('same tile point maps to same screen coords before and after resize (zoom/pan unchanged)', () => {
    ...
    expectScreen(
      vp.tileToScreen(tile),
      vp.resize(1200, 900).tileToScreen(tile),
    );
  });
...
describe('createViewport — invalid construction guards', () => {
  ...
  it('throws when zoom is NaN', () => {
    expect(() =>
      createViewport({ width: 800, height: 600, zoom: NaN }),
    ).toThrow('zoom');
  });

  it('throws when zoom is Infinity', () => {
    expect(() =>
      createViewport({ width: 800, height: 600, zoom: Infinity }),
    ).toThrow('zoom');
  });
  ...
  it('throws when panX is NaN', () => {
    expect(() =>
      createViewport({ width: 800, height: 600, panX: NaN }),
    ).toThrow('panX');
  });

  it('throws when panY is NaN', () => {
    expect(() =>
      createViewport({ width: 800, height: 600, panY: NaN }),
    ).toThrow('panY');
  });

  it('throws when extent is 0', () => {
    expect(() =>
      createViewport({ width: 800, height: 600, extent: 0 }),
    ).toThrow('extent');
  });

  it('throws when extent is negative', () => {
    expect(() =>
      createViewport({ width: 800, height: 600, extent: -1 }),
    ).toThrow('extent');
  });
