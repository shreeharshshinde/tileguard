...
vi.mock('../src/analysis/AnalysisAdapter.js', () => ({
  loadTileSnapshot: vi.fn().mockImplementation(async (filePath: string) => ({
    ...
    statistics: {
      ...
      layers: [
        { name: 'water', featureCount: 5, geometryCounts: { point: 0, line: 0, polygon: 5 }, diagnosticCount: 0 },
        { name: 'roads', featureCount: 15, geometryCounts: { point: 0, line: 15, polygon: 0 }, diagnosticCount: 0 },
        { name: 'buildings', featureCount: 30, geometryCounts: { point: 0, line: 0, polygon: 30 }, diagnosticCount: 0 },
      ],
    },
    layers: [
      { name: 'water', featureCount: 5, geometryCounts: { point: 0, line: 0, polygon: 5 }, extent: 4096 },
      { name: 'roads', featureCount: 15, geometryCounts: { point: 0, line: 15, polygon: 0 }, extent: 4096 },
      { name: 'buildings', featureCount: 30, geometryCounts: { point: 0, line: 0, polygon: 30 }, extent: 4096 },
    ],
    features: [
      ...Array.from({ length: 5 }, (_, i) => ({
        layerName: 'water', featureIndex: i, id: i,
        geometryType: 'Polygon', properties: {},
        geometry: [Array.from({ length: 20 }, (__, j) => ({ x: j, y: j }))],
      })),
      ...Array.from({ length: 15 }, (_, i) => ({
        layerName: 'roads', featureIndex: i, id: i + 5,
        geometryType: 'LineString', properties: { name: `road_${i}` },
        geometry: [Array.from({ length: 10 }, (__, j) => ({ x: j, y: j }))],
      })),
      ...Array.from({ length: 30 }, (_, i) => ({
        layerName: 'buildings', featureIndex: i, id: i + 20,
        geometryType: 'Polygon', properties: { height: i * 3 },
        geometry: [Array.from({ length: 8 }, (__, j) => ({ x: j, y: j }))],
      })),
    ],
    diagnostics: [
      { ruleId: 'tile/self-intersection', severity: 'error', message: 'Ring intersects', location: {} },
      { ruleId: 'tile/unclosed-ring', severity: 'warning', message: 'Ring not closed', location: {} },
      { ruleId: 'tile/self-intersection', severity: 'error', message: 'Another intersection', location: {} },
    ],
  })),
  getSnapshotStats: vi.fn().mockImplementation(() => ({
    layerCount: 3, featureCount: 50, vertexCount: 490, diagnosticCount: 3,
  })),
}));
...
describe('stats command', () => {
  it('returns exitCode 0', async () => {
    const result = await runStats({ file: 'test.pbf', format: 'text' }, makeCtx());
  ...
  it('json format returns valid JSON', async () => {
    const result = await runStats({ file: 'test.pbf', format: 'json' }, makeCtx());
  ...
  it('text output contains file name', async () => {
    const result = await runStats({ file: 'my-tile.pbf', format: 'text' }, makeCtx());
